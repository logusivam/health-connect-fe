...
export const updatePatientProfile = async (req, res) => {
  try {
    ...
    if (!updatedProfile) updatedProfile = await PatientProfile.findOne({ user_id: req.user.id });
    const updatedUser = await User.findById(req.user.id).select('email');

    // --- AUDIT LOG: Patient Profile Update ---
    AuditService.logAction(req, {
      action_type: 'UPDATE',
      entity_type: 'patient',
      entity_id: updatedProfile._id,
      new_values: { ...profileUpdateData, email }
    });
...
// Book Appointment / Create Treatment Record 
export const bookAppointment = async (req, res) => {
  try {
    ...
    AuditService.logAction(req, {
      ...
      new_values: { doctor_id, visitDate, chiefComplaint }
    });
...
export const getPatientAppointments = async (req, res) => {
  try {
    ...
    const records = await TreatmentRecord.find({ patient_id: patientProfile._id, is_deleted: false })
...
export const getPatientFlags = async (req, res) => {
  try {
    ...
    const flags = await UnsuitableMedicine.find({ patient_id: patientProfile._id, is_active: true })
