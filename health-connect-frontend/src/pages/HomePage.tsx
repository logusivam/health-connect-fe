...
export default function HomePage() {
  ...
  useEffect(() => {
    // 1. Dynamically inject the CSS
    const styleLink = document.createElement('link');
    styleLink.rel = 'stylesheet';
    styleLink.href = '/assets/css/style.css';
    
    // 2. Wait for the CSS to fully load before showing the content or running the JS
    styleLink.onload = () => {
      setIsReady(true);
      
      // Inject JS ONLY after the CSS is parsed. 
      // This prevents GSAP from calculating wrong dimensions on unstyled elements.
    ...
    return () => {
      ...
      // Safely remove all injected scripts
      const scripts = document.querySelectorAll('script[src="/assets/js/script.js"]');
  ...
  return (
    <>
      {/* LOADER - Inline styles ensure it instantly covers the screen, preventing the white flash */}
      <div 
        id="loader" 
        style={{ 
      ...
      >
        {/* Render loader contents only when CSS is ready so they don't flash unstyled */}
        {isReady && (
          <>
      ...
      {/* Only mount the rest of the DOM when CSS is ready to ensure perfect styling and JS targeting */}
      {isReady && (
        <>
          {/* CURSOR */}
          <div id="c-dot"></div>
