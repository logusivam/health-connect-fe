...
import { Filter, Search, User, X, Calendar } from 'lucide-react';
import type { PatientHistoryRecord } from '../../types/doctor.types';
import { mockPatientHistory, mockDoctor } from '../../data/mockDoctorData';
...
const TreatmentHistoryView: React.FC = () => {
  ...
  const filteredHistory = mockPatientHistory.filter(record => {
    // Apply "Treated by me" filter
    if (treatedByMe && !record.doctors.some(doc => doc.id === mockDoctor.id)) {
      return false;
    }
    ...
    return (
      ...
      record.department.some(dep => dep.toLowerCase().includes(query)) ||
      record.doctors.some(doc => doc.name.toLowerCase().includes(query) || doc.id.toLowerCase().includes(query))
    );
  });
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredHistory.length === 0 ? (
                ...
                >
                  ...
                  <td className="px-6 py-4 text-slate-600 max-w-[200px] truncate" title={record.diagnosis}>
                    {record.diagnosis}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex -space-x-2 overflow-hidden" title={record.doctors.map(d => d.name).join(', ')}>
                      {record.doctors.map((doc, i) => (
                        doc.avatar ? (
                          <img key={i} src={doc.avatar} alt={doc.name} className="inline-block h-8 w-8 rounded-full ring-2 ring-white object-cover bg-white" />
                        ) : (
                          <div key={i} className="inline-flex h-8 w-8 items-center justify-center rounded-full ring-2 ring-white bg-slate-200 text-slate-600 text-xs font-bold">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-8">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {selectedRecord.department.map(dep => (
              ...
              <div className="sm:col-span-2">
                ...
                <div className="flex flex-wrap gap-3">
                  {selectedRecord.doctors.map(doc => (
                    <div key={doc.id} className={`flex items-center gap-3 px-3 py-2 rounded-xl border ${doc.id === mockDoctor.id ? 'bg-blue-50 border-blue-200' : 'bg-slate-50 border-slate-200'}`}>
                      {doc.avatar ? (
                        <img src={doc.avatar} alt={doc.name} className="w-8 h-8 rounded-full object-cover shadow-sm" />
                      ) : (
                        <div className="w-8 h-8 rounded-full bg-white flex items-center justify-center text-slate-600 font-bold text-xs shadow-sm border border-slate-100">
                          {doc.name.replace('Dr. ', '').charAt(0)}
                        </div>
                      )}
                      <div>
                        <p className={`text-sm font-bold ${doc.id === mockDoctor.id ? 'text-blue-900' : 'text-slate-800'}`}>
                          {doc.name} {doc.id === mockDoctor.id && <span className="ml-1 text-blue-500 font-normal">(You)</span>}
                        </p>
