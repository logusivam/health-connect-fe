...
import { Filter, Search, User, X, Calendar, ChevronLeft, ChevronRight, AlertTriangle, Ban } from 'lucide-react';
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500">Global patient treatment history database.</p>
        </div>
      </div>
      ...
      {selectedRecord && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl max-w-2xl w-full p-6 sm:p-8 shadow-2xl relative animate-in fade-in zoom-in-95 duration-200 max-h-[90vh] flex flex-col">
            <button 
            ...
            <div className="overflow-y-auto pr-2 pb-4 -mr-2">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-8">
                ...
                {/* NEW: Unsuitable Medicines Section */}
                {selectedRecord.unsuitableMedicines && selectedRecord.unsuitableMedicines.length > 0 && (
                  <div className="sm:col-span-2 mt-2 pt-6 border-t border-slate-100">
                    <p className="text-xs font-bold text-red-500 uppercase tracking-wider mb-3 flex items-center gap-1.5">
                      <AlertTriangle className="w-4 h-4" /> Unsuitable / Allergic Medicines
                    </p>
                    <div className="grid grid-cols-1 gap-3">
                      {selectedRecord.unsuitableMedicines.map((med: any, i: number) => (
                        <div key={i} className="flex flex-col p-4 rounded-xl border border-red-100 bg-red-50/50 shadow-sm">
                          <div className="flex justify-between items-start mb-2">
                            <div>
                              <p className="font-bold text-red-900 text-base">{med.medicineName}</p>
                              <p className="text-xs font-semibold text-red-700 mt-0.5">Severity: {med.severity}</p>
                            </div>
                            <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider bg-red-100 text-red-700 border border-red-200">
                              <Ban className="w-3 h-3"/> Unsuit
                            </span>
                          </div>
                          <p className="text-sm text-red-800/80 mb-4 line-clamp-2" title={med.reason}>{med.reason}</p>
                          <div className="flex items-center gap-2 mt-auto pt-3 border-t border-red-100/60">
                            {med.doctor.avatar ? (
                              <img src={med.doctor.avatar} alt={med.doctor.name} className="w-6 h-6 rounded-full object-cover border border-red-100" />
                            ) : (
                              <div className="w-6 h-6 rounded-full bg-white flex items-center justify-center text-red-600 font-bold text-[9px] border border-red-200 shadow-sm">
                                {med.doctor.name.replace('Dr. ', '').charAt(0)}
                              </div>
                            )}
                            <p className="text-xs font-semibold text-red-900">
                              {med.doctor.name} 
                              <span className="text-red-700/70 font-medium ml-1">({med.doctor.specialization})</span>
                            </p>
                          </div>
                        </div>
                      ))}
